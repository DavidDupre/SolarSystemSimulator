...
public class ScenarioLoader {
	...
	public void init() {
		...
		if(eEpoch != null && eEpoch.hasAttributes() && !eEpoch.getAttribute("yr").equals("")) {
	...
	private void loadObjects() {
		for(Element e : editor.getObjectElements()) {			
			...
			case "orbit":
				...
				break;
			case "vector":
				Element posElement = (Element) eState.getElementsByTagName("pos").item(0);
				Vector3D pos = getVectorFromElement(posElement);
				Element velElement = (Element) eState.getElementsByTagName("vel").item(0);
				Vector3D vel = getVectorFromElement(velElement);
				state[0] = pos;
				state[1] = vel;
				break;
			case "tle":
				Element eLine1 = (Element) eState.getElementsByTagName("line1").item(0);
				Element eLine2 = (Element) eState.getElementsByTagName("line2").item(0);
				TLE tle = new TLE(name, eLine1.getTextContent(), eLine2.getTextContent());
				orb = tle.getOrbit();
				state = Astrophysics.toRV(orb, parent.mu, true);
				break;
			}
